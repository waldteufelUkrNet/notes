...
new_snippet                створює новий буфер та вставвляє в нього шаблон
                           сніпета
...
Enable Package                 вклічення пакунку
...
Команда / ім'я команди / що робить / Дефолтна комбінація клавіш / Моя комбінація клавіш
...
BracketHighlighter             підсвітка дужок, лапок і тегів
                               ...
                               показує, де закривається і відкривається  тег (
                               підкреслення тегу та маркування на полях).
...
css3_hex_convert Convert RGB Color To Hex


##### StyleToken

Додає до контекстного меню виділеного фрагменту пункт меню Style Token з 5-ма
передналаштованими стилями. Підтримує до 10 стилів (StyleToken.sublime-settings).
За замовчуванням клавіатурні скорочення має тільки для Windows.
...
Пакунок додає пункти меню Edit ▶ Trailing Space.
...
toggle_trailing_spaces                     Toggle Trailing Spaces Highlighting
                                           вмикає/вимикає підсвітку пробілів

edit_settings                              Trailing Spaces Settings
                                           відкриває файл налаштувань пакунку
                                           args:
                                             base_file: шлях до програмного
                                               файлу налаштувань
                                             default: шлях до кастомного файлу,
                                               який перезаписує base_file
...
? В ST3 при наведенні на дужку спливава попап, в ST4 - ні. Це можна поправити?

Пакунок додає пункти меню Tools ▶ Packages ▶ BracketHighlighter та
...
bh_toggle_enable              Toggle Global Enable
                              вмикає/вимикає пакунок

bh_remove_brackets            Remove Brackets
                              видаляє дужки

swap_brackets                 Swap Brackets
                              ???

wrap_brackets                 Wrap Selections with Brackets
                              огортає виділення дужками (попап)

bh_offscreen_popup            Find Matching Offscreen Bracket
                              ??? (попап)

bh_async_key                  Match Brackets (ignore threshold)
                              Jump to Left Bracket
                              Jump to Right Bracket
                              Select Bracket Content
                              Select Bracket Content with Brackets
                              Fold Bracket Content
                              Select Tag Name (closing and opening)
                              Select Next Attribute (right)
                              Select Next Attribute (left)
                              Swap Quotes
                              args:
...
bh_toggle_string_escape_mode  Toggle String Bracket Escape Mode
                              перемикається між режимами рядок/регулярка

bh_toggle_high_visibility     Toggle High Visibility Mode
                              перемикає режим підвищеної видимості

bracket_highlighter_open_site Documentation
                              відкриває сайт пакунку
                              args:
                                url: адреса сайта

bracket_highlighter_doc       Quick Start Guide
                              відкриває md-файл з початковою інструкцією
                              args:
                                page: адреса файлу

bh_debug                      Enable Debug Mode/Disable Debug Mode
