...
#### PackagesUI                                                 <i id="PUI"></i>

версія:      1.1.2
автор:       David Bekoyan
посилання:   https://packagecontrol.io/packages/PackagesUI
             https://github.com/unknownuser88/PackagesUI
призначення: показує список пакунків
залежності:  немає
...
посилання:   https://packagecontrol.io/packages/SublimeLinter
             ...
             https://sublimelinter.readthedocs.io/en/latest/index.html
...
можна змінити лише в налаштуваннях користувача.
...
призначення: підсвічує в коді технічні мітки як-то TODO, FIXME тощо
залежності:  ???
...
налаштувань SublimeLinter в секції "linters" по ключу "annotations".
...
залежності:  Nade.js, npm, htmlhint

Лінтер надає доступ до інтерфейсу HTMLHint.
...
###### Doctype/Head
* [ ] 
doctype-first        Документ повинен починатися з Doctype
                     Boolean

doctype-html5        Документ повинен починатися з Doctype, характерного для HTML
                     Boolean

html-lang-require    В HTML-тезі обов'язковий і валідний lang-фтрибут
                     Boolean

head-script-disabled Тег script не можна підключати в head
                     Boolean

style-disabled       не можна використовувати тег style в html-документі
                     Boolean

script-disabled      Не можна використовувати тег script в html-документі
                     Boolean

title-require        Тег title обо'язково присутній в head
                     Boolean


###### Attributes

attr-lowercase                 назви атрибутів повинні писатися малими літерами
                               Boolean або масив атрибутів-виключень, напр.
                               ["fooBar", "Test"]. Усі івдомі атрибути svg, як-то
                               viewBox ігноруються

attr-no-duplication            Відсутність дублікатів для атрибутів
                               Boolean

attr-no-unnecessary-whitespace Відсутність пробілів між назвою і значенням
                               атрибуту, напр. <div title ="a"></div>
                               Boolean

attr-unsafe-chars              Значення атрибутів не можуть містити небезпчні
                               символи, напр. \u0000-\u0009\u000b
                               Boolean

attr-value-double-quotes       Значення атрибуту повинно бути у подвійних лапках
                               Boolean

attr-value-single-quotes       Значення атрибуту повинно бути в одинирних лапках
                               Boolean

attr-value-not-empty           Атрибути повинні мати значення
                               Boolean

attr-sorted                    Атрибути повинні бути відсортовані по порядку:
                               class, id, name, src, for, type, href, value,
                               title, alt, role. Решта атрибутів ігнорується
                               Boolean

attr-whitespace                Значення атрибуту не повинно містити початкових
                               або кінцевих пробілів
                               Boolean

alt-require                    В тегах img, area[href] та input[type=image]
                               повинен бути присутній тег alt
                               Boolean

input-requires-label           Усі теги input повинні мати відповідні теги label,
                               точніше: не повинно бути кілька тегів label з
                               однаковими атрибутами for
                               Boolean


###### Tags

tags-check                Дозвіл на специфічні правила для тегів і їх валідація
                          Boolean

tag-pair                  Подвійні теги повинні мати закриваючий тег
                          Boolean

tag-self-close            Порожні теги мають бути закриті
                          Boolean

tagname-lowercase         Усі теги пишуться малими літерами
                          Boolean або масив тегів-виключень, напр.
                          ["clipPath", "test"]

tagname-specialchars      Назви тегів не повинні містити символів крім букв,
                          цифр, ```"-", ":" або "_"```.
                          Boolean

empty-tag-not-self-closed Порожній тег не може закриватися самостійно (напр.
                          ```html
                          <br />
                          ```)
                          Boolean

src-not-empty             src-атрибут img/script/link повинен мати значення
                          Boolean

href-abs-or-rel           Тип посилань
                          "abs" - абсолютні
                          "rel" - відносні
                          false - правило вимкнене


###### Id

id-class-ad-disabled id та класи не можуть використовувати слово "ad"/"ad-"
                     (такі назви можуть блокуватися антирекламними програмами)
                     Boolean

id-class-value       Значення id та класів повинні відповідати певним вимогам
                     "underline" - aaa_bbb
                     "dash" - aaa-bbb
                     "humb" - aaaBbb
                     false - правило вимкнене

id-unique            id-атрибут повинен бути унікальним в документі
                     Boolean 


###### Inline

inline-script-disabled Заборона на використання inline-скриптів, напр.:
                       ```html
...
inline-style-disabled  Заборона на використання inline-стилів
                       Boolean


###### Formatting

space-tab-mixed-disabled Відступи формуються або пробілами, або табами
                         "space" - відступи тільки з пробілів
                         "space4" - відступи по 4 пробіли (або таби, але не на
                                    одному рядку)
                         "tab" - відступи тільки з табів
                         false - правило виключене

spec-char-escape         Спеціальні символи повинні бути екрановані (напр. >)
                         Boolean
...
призначення: надає можливість перевіряти правильність синтаксису markdown та
             CommonMark
залежності:  Node.js, npm, markdownlint
...
first-header-h1                   (first-heading-h1) Визначає, яким рівнем
                                  заголовку повинен починатися документ. Правило
                                  застаріле, натомість треба використовувати
                                  правило first-line-heading.
                                  {"level": 1}

header-style                      (heading-style) Не можна використовувати різні
                                  стилі заголовків, напр.:
                                  ```md
                                  # ATX style H1
                                  ## Closed ATX style H2 ##
                                  Setext style H1
                                  ...
                                  ```
                                    atx - стиль atx
                                    atx_closed - стиль atx із закриваючим тегом
                                    consistent - послідовний?
                                    setext - стиль setex
                                    setext_with_atx - стиль setex + atx
                                    setext_with_atx_closed - стиль setex + atx closed
                                  {"style":"consistent"}

ul-style                          Визначає стиль не нумерованих списків
                                    asterisk
                                    consistent
                                    dash
                                    plus
                                    sublist - різні значки на різних рівнях
                                  {"style": "consistent"}

list-indent                       Забороняє не однакові відступи для списків
...
ul-start-left                     Забороняє робити відступи перед списком.
                                  Правило застаріле, замість нього треба
                                  використовувати ul-indent
                                  true

ul-indent                         Впорядковує відступи у вкладених списках.
                                    start_indented - чи потрібен відступ для
                                                     списку
                                    start_indent - початковий відступ для списку
                                    indent - відступи для вкладених списків
                                  {
                                    "indent": 2,
...
no-trailing-spaces                Порядок обробки прикінцевих пробілів
                                    br_spaces - робить виняток з привила для
                                                певної кількості пробілів, які
                                                позначають явний розрив рядку
                                                (тег br), має бути 2 або більше,
                                                інакше правило не працює
                                    list_item_empty_lines - Using spaces to
                                                indent blank lines inside a list
                                                item is usually not necessary,
                                                but some parsers require it.
                                    strict - більш сворий режим відстеження
                                             br_spaces
                                  {
                                    "br_spaces": 2,
...
no-hard-tabs                      Забороняє використовувати вирівнювання табами
                                    code_blocks - false, якщо потрібно вимкнути
                                                  правило для блоків з кодом
                                    ignore_code_languages - масив з мовами, яким
                                                            дозволені таби
                                    spaces_per_tab - скільки пробілів міститься
                                                     в табі
                                  {
                                    "code_blocks": true,
...
no-reversed-links                 Перевіряє правильність послідовності дужок у
                                  посиланнях
                                  true

no-multiple-blanks                Регулює кількість послідовних порожніх рядків
                                  {"maximum": 1}

line-length                       Правило визначає довжину рядків. Має виключення
                                  для довгих рядків без пробілів в частині, яка
                                  вилазить за межу.
                                    code_block_line_length - довжина блоків коду
                                    code_blocks - по аналогії до no-hard-tabs?
                                    headers - Include headings?
                                    heading_line_length - довжина заголовків
                                    headings - Include headings?
                                    line_length - максимальна довжина рядка
                                    stern - попередити про довгі рядки, але дозолити довгі рядки без пробілів
                                    strict - виключає обмеження для довгих рядків без пробілів
...
commands-show-output              Забороняє $ на початку рядків у блоках коду,
                                  якщо усі рідки починаються з нього
                                  true

no-missing-space-atx              Показує, що в загаловку після # нема пробілу
                                  true

no-multiple-space-atx             Показує, що в заголовку після # кілька пробілів
                                  true

no-missing-space-closed-atx       Показує, що в закритому atx-стилі між # та
                                  заголовком нема пробілу
                                  true

no-multiple-space-closed-atx      Показує, що в закритому atx-стилі між # та
                                  заголовком кільак пробілів
                                  true

blanks-around-headers             (blanks-around-headings) Визначає кількість
...
header-start-left                 (heading-start-left) Заголовок повинен
                                  починатися з початку рядка
                                  true

no-duplicate-header               (no-duplicate-heading) Забороняє однакові
                                  назви в заголовках
                                    allow_different_nesting - тільки сусідні
                                    siblings_only - тільки сусідні (дублікат?)
                                  {
                                    "allow_different_nesting": false,
                                    "siblings_only": false
                                  }

single-h1                         (single-title) Забороняє використовувати
                                  кілька заголовків верхнього рівня у одному
                                  документі
                                    level - рівень верхнього заголовка
                                    front_matter_title - регулярка для заголовка
                                  {
                                    "level": 1,
                                    "front_matter_title": "^\\s*title\\s*[:=]"
                                  }

no-trailing-punctuation           Забороняє прикінцеві знаки в заголовках
                                  {"punctuation": ".,;:!。，；：！"}

no-multiple-space-blockquote      Забороняє більше одного пробіду після символу
                                  > (він позначає цитати - blockquote)
                                  true

no-blanks-blockquote              Забороняє розрив цитати порожнім рядком, напр.:
                                  ```md
...
ol-prefix                         Контролює правильність нумерації нумерованого
                                  списку, дозволяє перед цифрами ставити 0 для
                                  вирівнювання нумерації
                                  one - усі елементи списку мають один номер
                                  ordered - нумерація зростає від 1 або 0
                                  one_or_ordered - або one або ordered
                                  zero - усі елементи списку починаються з 0
...
list-marker-space                 Контролює пробіли після маркерів елементів
                                  списку
                                  ol_multi - пробіли у вкладених нумерованих списках
                                  ol_single - пробіли у простих нумерованих списках
                                  ul_multi - пробіли у вкладених ненумерованих списках
                                  ul_single - пробіли у простих ненумерованих списках
                                  {
                                    "ul_single": 1,
                                    "ol_single": 1,
                                    "ul_multi": 1,
...
blanks-around-fences              Перевіряє відмежування блоків коду пустими
                                  рядками
                                  {"list_items": true}

blanks-around-lists               Перевіряє відмежування списків пустими рядками
                                  true

no-inline-html                    Забороняє використовувати HTML-теги
                                  {"allowed_elements": []} 

no-bare-urls                      Забороняє використання url, не огорнених у <>
                                  або `` або []()
                                  true

hr-style                          Перевіряє правильність горизонтальних ліній
                                  {"style": "consistent"}

no-emphasis-as-header             (no-emphasis-as-heading) Контролює, щоб замість
                                  заголовків не використовувався жирний текст або
                                  курсив
                                  {"punctuation": ".,;:!?。，；：！？"}

no-space-in-emphasis              Забороняє пробіли між текстом і сииволами
                                  жирного або курсивного текстів
                                  true

no-space-in-code                  Забороняє пробіли між інлайновим кодом і
                                  одинарними зворотніми лакками, напр.: ` code `
                                  true

no-space-in-links                 Забороняє пробіли у квадртних дужках у
                                  посиланнях, напр.: [ text ](link)
                                  true

fenced-code-language              Блоки коду повинні містити інформацію про мову,
...
first-line-h1                     (first-line-heading) Перший рядок в документі
                                  повинен бути заголовком вищого рівня
                                  level - dfkslybq hsdtym pfujkjdre
                                    front_matter_title - регулярка, яка дозволяє
                                                         як заголовок використо-
                                                         вувати картинку (часта
                                                         практика на github)
                                  {
                                    "level": 1,
                                    "front_matter_title": "^\\s*title\\s*[:=]"
...
no-empty-links                    Забороняє порожні посилання, напр.: [text]()
                                  true

required-headers                  (required-headings) Контролює структуру
...
proper-names                      Перевіряє правильність написання окремих слів
                                  (капіталізація букв)
                                    names - масив слів, які потрібно писати
                                            правильно, напр.: `["JavaScript"]`
                                    code_blocks - false вимикає це правило для
                                                  блоків і рядків коду
                                    html_elements - false вимикає це правило для
...
no-alt-text                       Контролює наявність альтернативного тексту в
                                  картинках, напр.: 
                                  `![Alternate text](ref)` або
                                  `![Alternate text][ref]
                                  ...
                                  [ref]: image.jpg "Optional title"`
                                  true

code-block-style                  Контролює однаковість написання блоків коду
                                    consistent - послідовний?
                                    fenced - огорнутий в ```code```
                                    indented - позначений відступами
                                  {"style": "consistent"}

single-trailing-newline           Правило спрацьовує, коли в кінці файлу нема
                                  пустого рядка
                                  true

code-fence-style                  визначає символи позначення коду
                                    backtick - ```code```
                                    consistent - послідовний?
                                    tilde - ~~~code~~~
                                  {"style": "consistent"}

emphasis-style                    Контролює однаковість позначення курсиву
                                    asterisk - *text*
                                    consistent - послідовний?
                                    underscore - _text_
                                  {"style": "consistent"}

strong-style                      Контролює однаковість позначення жирного тексту
                                    asterisk - **text**
                                    consistent - послідовний?
                                    underscore - __text__
                                  {"style": "consistent"}

link-fragments                    фрагменти посилань повинні відповідати імнам
...
reference-links-images            Правило попереджає про відсутність label в
...
link-image-reference-definitions  Позначає зайві label в тексті
                                  {"ignored_definitions": ["//"]}
